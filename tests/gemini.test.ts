...
describe("resolveModels", () => {
  it("has nothing of its own to fall back on", () => {
    expect(gemini.resolveModels(undefined, undefined)).toEqual([])
    expect(gemini.resolveModels("", "  ")).toEqual([])
  })

  it("reads GEMINI_MODELS as the chain, in order", () => {
    expect(gemini.resolveModels(" a , b ,, c ", "ignored")).toEqual(["a", "b", "c"])
  })

  it("reads GEMINI_MODEL as a chain of one", () => {
    expect(gemini.resolveModels(undefined, "gemini-3.5-flash")).toEqual(["gemini-3.5-flash"])
  })

  it("is not enabled by a key alone, with no model to spend it on", async () => {
    const models = process.env.GEMINI_MODELS
    process.env.GEMINI_MODELS = ""
    vi.resetModules()
    const bare = await import("../src/services/gemini.js")
    expect(bare.geminiEnabled()).toBe(false)
...
describe("the fallback chain", () => {
  ...
  /*
   * The one that took a production run down: a slow model rejected with a
   * TimeoutError, which is not a GeminiError, and the whole question stopped
   * on it while the second model still had budget.
   */
  it("moves to the next model when one runs out of time", async () => {
    const timeout = () => {
  ...
  it("still gives up when every model runs out of time", async () => {
    fetchMock.mockImplementation(() => {
      const error = new Error("The operation was aborted due to timeout")
      error.name = "TimeoutError"
      return Promise.reject(error)
    })
    await expect(call()).rejects.toThrow(/aborted due to timeout/)
    expect(fetchMock).toHaveBeenCalledTimes(CHAIN.length)
  })
...
/*
 * Between questions, not inside one. A model that has just refused or hung is
 * skipped for a while, because the question after it used to walk straight
 * back into the same wall and pay the same twenty seconds for the news.
 */
describe("what the chain remembers", () => {
  const timeout = () => {
    const error = new Error("The operation was aborted due to timeout")
    error.name = "TimeoutError"
    return Promise.reject(error)
  ...
  it("skips a model that timed out on the question before", async () => {
    fetchMock
      .mockImplementationOnce(timeout)
      .mockResolvedValueOnce(okResponse({ transactions: [] }))
    await expect(answer()).resolves.toEqual({ transactions: [] })
    ...
    // Straight to the second model: the first is still cooling.
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(modelOf(fetchMock.mock.calls[0]!)).toBe(CHAIN[1])
  })

  it("skips a model that answered 503, for every key", async () => {
    fetchMock
      .mockImplementationOnce(errorResponse(503))
      .mockResolvedValueOnce(okResponse({ transactions: [] }))
    await expect(answer()).resolves.toEqual({ transactions: [] })
  ...
  it("does not hold a whole model against one key running out of quota", async () => {
    fetchMock
    ...
    fetchMock.mockResolvedValue(okResponse({ transactions: [] }))
    await answer()
    // One key, so the pair is cooling and the next model answers — but the
    // model itself was never cooled, which is the difference being asserted.
    expect(modelOf(fetchMock.mock.calls[0]!)).toBe(CHAIN[1])
    expect(fetchMock).toHaveBeenCalledTimes(1)
  })

  it("asks anyway when every model is cooling, rather than refusing untried", async () => {
    fetchMock.mockImplementation(timeout)
    await expect(call()).rejects.toThrow(/aborted due to timeout/)
    expect(fetchMock).toHaveBeenCalledTimes(CHAIN.length)
    ...
    fetchMock.mockResolvedValue(okResponse({ transactions: [] }))
    await expect(answer()).resolves.toEqual({ transactions: [] })
    // A cooldown is a guess, and someone is waiting: one attempt, not none.
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(modelOf(fetchMock.mock.calls[0]!)).toBe(CHAIN[0])
  })
})
